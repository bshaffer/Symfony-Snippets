 ...
 */
class csWidgetFormTime extends sfWidgetForm
{
  /**
   * Constructor.
   *
  ...
  {
    $with_meridiem = $this->getOption('with_meridiem');
    ...
    else
    {
      ...
      else
      {
        ...
        $value = array('hour' => (int) date($with_meridiem ? 'h' : 'H', $value), 'minute' => (int) date('i', $value), 'second' => (int) date('s', $value), 'meridiem' => date('a', $value));
      }
    }
    ...
    {
      // AM / PM
      $widget = new sfWidgetFormSelect(array('choices' => $this->getOption('can_be_empty') ? array('' => $emptyValues['meridiem']) + $this->getOption('meridiem') : $this->getOption('meridiem')), array_merge($this->attributes, $attributes));
